...
static int nvme_cmd(nvme_ctrl_t* c, int qid, nvme_sqe_t* sqe_in) {
    ...
    *tail = (*tail + 1) % 64; nvme_db(c, qid, 0, *tail);
    int timeout = 1000000;
    while(--timeout > 0) {
...
void nvme_init(uint64_t mmio) {
    if(!mmio) return; nvme_ctrl_t* c = &g_nvme; c->regs = (volatile uint32_t*)(mmio + hhdm_offset);
    serial_printf("[NVMe] Initializing Sovereign Controller at %lx\n", mmio);
    c->regs[NVME_REG_CC/4] &= ~1;
    int timeout = 1000000; while((c->regs[NVME_REG_CSTS/4] & 1) && --timeout > 0) __asm__("pause");
    if (timeout <= 0) { serial_printf("[NVMe] ERROR: Reset Timeout.\n"); return; }
    ...
    c->regs[NVME_REG_CC/4] = (0<<16)|(6<<12)|(4<<7)|1;
    timeout = 1000000; while(!(c->regs[NVME_REG_CSTS/4] & 1) && --timeout > 0) __asm__("pause");
    if (timeout <= 0) { serial_printf("[NVMe] ERROR: Enable Timeout.\n"); return; }
    ...
    nvme_sqe_t cq_sqe = { .cdw0=0x05, .prp1=(uintptr_t)c->io_cq-hhdm_offset, .cdw10=(63<<16)|1, .cdw11=1 };
    if (nvme_cmd(c, 0, &cq_sqe) != 0) { serial_printf("[NVMe] ERROR: CQ Create failed.\n"); return; }
    nvme_sqe_t sq_sqe = { .cdw0=0x01, .prp1=(uintptr_t)c->io_sq-hhdm_offset, .cdw10=(63<<16)|1, .cdw11=1 };
    if (nvme_cmd(c, 0, &sq_sqe) != 0) { serial_printf("[NVMe] ERROR: SQ Create failed.\n"); return; }
    uint8_t ns_id[4096]; nvme_sqe_t id_sqe = { .cdw0=0x06, .nsid=1, .prp1=(uintptr_t)ns_id-hhdm_offset, .cdw10=0 };
    if (nvme_cmd(c, 0, &id_sqe) != 0) { serial_printf("[NVMe] ERROR: Namespace Identify failed.\n"); return; }
    ...
    devmgr_register_storage(&nvme_dev);
}
