...
static void ps2_keyboard_handler(struct cpu_state *state) {
    (void)state;
...
static void ps2_mouse_handler(struct cpu_state *state) {
    (void)state;
...
void hal_ps2_init(void) {
    serial_printf("[PS2] Initializing i8042 controller...\n");
    ...
    irq_install_handler(32 + KBD_IRQ, ps2_keyboard_handler);
    irq_install_handler(32 + MOUSE_IRQ, ps2_mouse_handler);
}
