/* Modified by Sovereign: Meaty Preemptive Scheduler with Context Switching */
#include "pro_os.h"
#include <string.h>
...
void scheduler_add_task(const char *name, void (*entry)(void)) {
    if (task_count < MAX_TASKS) {
        ...
        uint64_t *stack = (uint64_t *)&task_stacks[task_count][STACK_SIZE];
        ...
        /* iretq frame */
        *(--stack) = 0x10; /* SS */
        *(--stack) = (uint64_t)&task_stacks[task_count][STACK_SIZE - 8]; /* RSP */
