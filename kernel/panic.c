...
static void panic_printf(struct panic_framebuffer* fb, const char* fmt, uint64_t a1, uint64_t a2) {
    ...
    while (*fmt) {
        if (*fmt == '%' && *(fmt+1)) {
            ...
            if (*fmt == 's') { const char* s = (const char*)v; while(*s) { serial_putc(*s); if (cx+16>(int)fb->width) {cx=40; cy+=24;} raw_char(fb, *s++, cx, cy, 0xFFFFFF); cx+=16; } }
            else if (*fmt == 'x') { const char* h = "0123456789ABCDEF"; for (int i=15; i>=0; i--) { char c = h[(v>>(i*4))&0xF]; serial_putc(c); if(cx+16>(int)fb->width) {cx=40; cy+=24;} raw_char(fb, c, cx, cy, 0xFFFFFF); cx+=16; } }
        } else if (*fmt == '\n') { serial_putc('\n'); cx=40; cy+=24; }
        else { serial_putc(*fmt); if(cx+16>(int)fb->width) {cx=40; cy+=24;} raw_char(fb, *fmt, cx, cy, 0xFFFFFF); cx+=16; }
        fmt++;
    }
}
